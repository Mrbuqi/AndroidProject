...
public class AnimUtils {
    ...
    public static Animator animateIn(View view,int dd ,Style style, Animator.AnimatorListener listener) {
        switch (style) {
            case Fade:
    ...
    public static Animator animateIn(View view, Style style, Animator.AnimatorListener listener) {
        switch (style) {
            ...
            case Pop:
                return popIn(view, listener);
            case Fly:
                return flyIn(view,200, listener);
            case BrightnessSaturationFade:
                return view instanceof ImageView ? brightnessSaturationFadeIn((ImageView) view, listener) : fadeIn(view, listener);
            case ProgressWidth:
                return view instanceof ProgressBar ? progressWidthIn((ProgressBar) view, listener) : fadeIn(view, listener);
    ...
    public static ValueAnimator flyIn(final View view, int dd,Animator.AnimatorListener listener) {
        if (view.getVisibility() != View.VISIBLE)
        ViewHelper.setAlpha(view, 0);
        view.setTranslationY(dd);
        view.setAlpha(0.0f);
...
//        });
    ...
    public static ValueAnimator flyOut(final View view, Animator.AnimatorListener listener) {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ViewHelper.setAlpha(view, (Float) valueAnimator.getAnimatedValue());
                ViewHelper.setTranslationY(view, Math.min(view.getHeight() , view.getResources().getDimension(R.dimen.carbon_1dip) * 50.0f) * (1 - (Float) valueAnimator.getAnimatedValue()));
                if (view.getParent() != null)
                    ((View) view.getParent()).postInvalidate();
            }
        });
