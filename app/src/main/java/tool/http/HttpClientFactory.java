...
/**
 * 客户端所有http请求共用一个httpclient。 该DefaultHttpClient实例是线程安全的可以放心使用。
 * 
 * @author zoubangyue
 */
public final class HttpClientFactory {
	...
	/**
	 * 使用线程安全的http客户端管理器，管理来自不同的请求使用一个httpClient。 注册客户端的协议，字符编码，请求端口等。
	 * 
	 * @return
	...
	private static DefaultHttpClient createHttpClient() {
		...
		try {
			...
			// 有太多的web站点是用CGI脚本去实现的，而导致只有将所有的Cookies都放入Request
			// header才可以正常的工作。这种情况下最好设
			// 置http.protocol.single-cookie-header参数为true。
