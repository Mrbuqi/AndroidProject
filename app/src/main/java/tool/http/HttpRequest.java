...
import tool.http.upload.CountMultipartEntity;

/**
 * httpclient请求。 1.支持get,post两种请求方式 2.支持普通请求响应字符串数据，上传文件、流，下载文件
...
public class HttpRequest implements HttpInterface {
	...
	public ReturnData doGetRequest(String url, Map<String, String> params) {
		try {
			...
			// System.out.println("doGet-url-utf8:"+ url + "?" + s);
			// String encodeQuery = URLEncoder.encode("doGet-url"+ url + "?" +
			// HttpUtils.getParamData(params),
			// "UTF-8");
			String paramData = HttpUtils.getParamData(params);
			if (null != paramData) {
				httpGet = new HttpGet(url + "?" + paramData + "");
				// httpGet = new HttpGet(url + "" +
				// HttpUtils.getParamData(params));
				System.out.println("doGet-url:" + url + "?" + paramData);
			...
			if (data.status == ReturnData.SC_OK) {
				// 记录流量
				// FlowController.count(FlowController.FLOW_UP,
				// data.contentLength);
			}
		}
		return data;
	...
	public ReturnData doPostRequest(String url, Map<String, String> params) {
		try {
			...
			// httpPost.setEntity(new UrlEncodedFormEntity(vp ,"gbk"));
	...
	/**
	 * post请求，下载文件流。 注意流处理完毕之后必须要调用consumeResponse方法。
	 * 
	 * @param url
	 *            地址
	 * @param params
	 ...
	 */
	@Override
	public ReturnData doDownloadRequest(String url, Map<String, String> params,
			int dataType) {
		try {
			// url = SessionCenter.addSessionToUrl(url);
			// List<NameValuePair> vp = HttpUtils.getNameValuePair(params);
	...
	/**
	 * get请求，下载文件流。 注意流处理完毕之后必须要调用consumeResponse方法。
	 * 
	 * @param url
	 *            地址
	 * @param params
	 ...
	 */
	@Override
	public ReturnData doGetDownloadRequest(String url,
			Map<String, String> params, int dataType) {
		try {
			// url = SessionCenter.addSessionToUrl(url);
			// List<NameValuePair> vp = HttpUtils.getNameValuePair(params);
			httpGet = new HttpGet(url);
	 ...
	 */
	@Override
	public ReturnData doPostUploadFileRequest(String url,
			Map<String, String> params, CountMultipartEntity.ProgressListener listener,
			File[] files, String[] fileNames) {
		try {
			// url = SessionCenter.addSessionToUrl(url);
			httpPost = new HttpPost(url);
			SessionCenter.putSessionToHeader(url, httpPost);
			CountMultipartEntity entity = new CountMultipartEntity(
					HttpMultipartMode.BROWSER_COMPATIBLE, "*****",
					Charset.forName(HTTP.UTF_8), listener);
			Set<String> keys = params.keySet();
			for (String key : keys) {
				StringBody paramValue = new StringBody(params.get(key),
	 ...
	 */
	@Override
	public ReturnData doPostUploadStreamRequest(String url,
			Map<String, String> params, CountMultipartEntity.ProgressListener listener,
