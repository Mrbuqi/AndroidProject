...
import tool.http.HttpRequest;
import tool.http.ReturnData;
...
public class Requester implements Runnable {
		...
		public void handleMessage(Message msg) {
			...
			if (requester.mRequestListener != null) {
				if (msg.what == RequestStatus.SUCCESS
					...
					Log.d("Requester", "mRequestUrl:" + requester.mRequestUrl
							+ ",what:" + msg.what + ",result:" + result);
				requester.mRequestListener.requestStatusChanged(msg.what,
						requester.mRequestId, result);
			}
			if (requester.mNewRequestListener != null) {
				if (msg.what == RequestStatus.SUCCESS
						|| msg.what == RequestStatus.FAIL)
					Log.d("Requester", "mRequestUrl:" + requester.mRequestUrl
							+ ",what:" + msg.what + ",result:" + result
							+ ",params:" + requester.mRequestParams);
				requester.mNewRequestListener.requestStatusChanged(msg.what,
						requester.mRequestId, result, requester.mRequestParams);
			}
			if (msg.what == RequestStatus.FAIL) {
				...
				// boolean isNetwork = Utily.isNetworkAvailable();
				// if (act.hasWindowFocus() && !isNetwork) {
				// MessageDialog dialog = new MessageDialog();
				// // 前台运行
				// dialog.showDialogOKCancel(
				// (Activity) requester.mContext,
				// "网络未连接，可否先连接网络？", new MessageDialogListener() {
				//
				// @Override
				// public void onMessageDialogClick(
				// MessageDialog dialog, int which) {
				...
				// });
				// } else {
				// // NewToast.show(requester.mContext, " (×_×) 请求超时",
				// // false);
				// }
				// }
			}
	 ...
	 * @param listener
	 * @param
	 *  ，如果调用端同时发送多个也去请求，可以在调用端维护唯一id，并传递，在业务处理结束后，带回
	 * @param requestUrl
	 */
	@Deprecated
	public void request(RequestListener listener, HttpRequestID requestId,
	 ...
	 */
	public void request(RequestListener listener, HttpRequestID requestId,
			String requestUrl, Map<String, String> params, int requestType,
			boolean needSession) {
	 ...
	 * @param listener
	 * @param
	 *  ，同时发送多个也去请求，可以在调用端维护唯一id，并传递，在业务处理结束后，带回
	 * @param requestUrl
	 * @param
	 * @param needSession
	 */
	public void request(NewRequestListener listener, HttpRequestID requestId,
			String requestUrl, Map<String, String> params, int requestType,
			boolean needSession) {

		this.mNewRequestListener = listener;
		this.mRequestParams = params;
		this.mRequestUrl = requestUrl;
		this.mRequestId = requestId;
		this.mRequestType = requestType;
	...
	@Override
	public void run() {
		mRequest = new HttpRequest(mContext);
		bCanceled = false;
		mNowRetryCount = 0;
